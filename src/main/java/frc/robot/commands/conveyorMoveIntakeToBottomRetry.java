 ...
 *
 */
public class conveyorMoveIntakeToBottomRetry extends Command {
    ...
    private double lastTimeChanged = 0; 
    ...
    protected void execute() {
        /* 2-21-2020
         *    As we decrease the distance that the ball is from the conveyor when we
         *    start moving the conveyor, we more frequently encounter a situation
         *    where the conveyor doesn't move because we are waiting for the previous
         *    ball to clear the sensor but the current ball is not longer in range
         *    of the "ready to load" sensor.
         * 
         *    Now the conveyor will complete the current ball unless the top sensor
         *    is triggered before the current ball reaches the bottom sensor
         */
	int convayorDirection = 1;
        if (!Robot.robotSensors.isTopInRange() &&
            (Robot.robotSensors.isIntakeInRange() || ballIsMoving)) {
            Robot.conveyor.move(convayorDirection*Robot.oi.getConveyorInput());
            ballIsMoving = true;
        }
        double currentTime = time();
        if(currentTime - lastTimeChanged > 0.5){
           convayorDirection = convayorDirection * -1;
