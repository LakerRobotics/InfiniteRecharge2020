...
public class DriveTrain extends Subsystem {
    ...
    private final double MINIMUM_VISION_TURN_SPEED = 0.7;           // Carpet Values       
    private final double MAXIMUM_VISION_TURN_SPEED = 0.5;          // Carpet Values
    ...
    private final double MINICIM_MAX_FREE_RPM = (5840 * 0.9) / 10.64;         // DOCUMENTED FREE SPEED MINUS 10% AND GEAR REDUCTION OF 10.64:1
    private final int MOTION_ACCELERATION = 500;
...
rightSPX2 = new WPI_VictorSPX(14);
    ...
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTORS
        ...
        leftSRX.setSelectedSensorPosition(0, 0, 30);
        ...
        */
        ...
        rightSRX.setSelectedSensorPosition(0, 0, 30);
    ...
    public void driveVelocity(double velocity, double rotation) {

        double currentVelocitySpeedControl = 0;
        double currentVelocityRotationControl = 0;
        double maxVelocity = 3700;
        double intervalPercent = 0.1;
        ...
        */
        ...
        /*
        if (Math.abs(velocity) > 0.2) currentVelocitySpeedControl = velocity;
        else currentVelocitySpeedControl = 0;
    
        if (Math.abs(rotation) > 0.2) currentVelocityRotationControl = rotation;
        else currentVelocityRotationControl = 0;

        double leftCurrentVelocity = leftSRX.getSelectedSensorVelocity(0);
        SmartDashboard.putNumber("Left Velocity", leftCurrentVelocity);
        double rightCurrentVelocity = rightSRX.getSelectedSensorVelocity(0);
        SmartDashboard.putNumber("Right Velocity", rightCurrentVelocity);

        double leftTargetVelocity = (velocity + rotation) * maxVelocity;
        double rightTargetVelocity = (velocity - rotation) * maxVelocity;

        double allowedVelocity = maxVelocity * intervalPercent; //3700 * 10% = 370
        double setTargetVelocity = maxVelocity - allowedVelocity; //3700 - 370 = 3330
        */


        leftSRX.set(ControlMode.Velocity, 2000);
        rightSRX.set(ControlMode.Velocity, 2000);
    ...
    public void driveStraightUsingEncoderPID(double inches, double speed) {
        ...
        // int acceleration = MOTION_ACCELERATION;
        ...
        SmartDashboard.putNumber("cruise", cruiseVelocity);
    ...
    public void aimUsingChassis() {
         ...
         * **********************************************************************/
        // diffDrive.tankDrive(leftSpeed, rightSpeed);
        // leftSRX.set(ControlMode.PercentOutput, turnSpeed);
    ...
    public boolean isRobotAimed() {
        ...
        if (Robot.robotSensors.getTV()) {
            SmartDashboard.putNumber("visionError", tx);
            SmartDashboard.putBoolean("visionFound", true);
            if (Math.abs(tx) < VISION_TOLERANCE) return true;
            else return false;
        }
        else {
            SmartDashboard.putBoolean("visionFound", false);
